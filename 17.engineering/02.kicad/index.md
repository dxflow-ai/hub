...
```bash
dxflow workflow create --identity kicad hub://kicad

# Start with defaults, or tune per run with --override
dxflow workflow start kicad
dxflow workflow start kicad \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start kicad --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. KiCad is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach KiCad from anywhere.
...
- The panel and taskbar are hidden by default so the project window fills the screen. Set `PANEL=show` or `TASKBAR=show` to bring back the window decorations and taskbar.
...
- The symbol, footprint, and 3D-model libraries are separate packages this image does not carry — `xbps-install -Sy kicad-library kicad-library-3d` inside the session to add them.
