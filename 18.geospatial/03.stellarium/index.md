...
```bash
dxflow workflow create --identity stellarium hub://stellarium

# Start with defaults, or tune per run with --override
dxflow workflow start stellarium
dxflow workflow start stellarium \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start stellarium --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. Stellarium is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach Stellarium from anywhere.
...
- The panel and taskbar are hidden by default so the sky view fills the screen. Set `PANEL=show` or `TASKBAR=show` to bring back the window decorations and taskbar.
...
- The sky renders through software OpenGL, so a large window costs CPU rather than GPU. Turn down the resolution or the star catalogue depth if it feels heavy.
