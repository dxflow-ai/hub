...
```bash
dxflow workflow create --identity grass hub://grass

# Start with defaults, or tune per run with --override
dxflow workflow start grass
dxflow workflow start grass \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start grass --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. GRASS GIS is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach GRASS GIS from anywhere.
...
- The panel and taskbar are hidden by default so the wxGUI fills the screen. Set `PANEL=show` or `TASKBAR=show` to bring back the window decorations and taskbar.
...
- It opens on the data selection screen: GRASS needs a database directory, a location, and a mapset before the map display is useful. Point it at a directory under `/volume` so the database survives a restart.
- This entry builds on the Ubuntu desktop rather than Void: Void's GRASS is orphaned at 8.3.0 and amd64 only, while Ubuntu carries 8.4.2 for both arches.
