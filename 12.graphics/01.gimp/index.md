---
title: GIMP
description: GNU Image Manipulation Program for photo editing and graphics
navigation:
    icon: i-diphyx:gimp
---

GIMP is a full-featured raster graphics editor for photo retouching, image composition, and authoring, streamed here in a remote desktop session and backed by remote compute. It builds on the [Void Desktop](/hub/desktop/void) image — GIMP launches maximized with the window decorations and taskbar hidden, so the editor fills the screen.
...
```bash
dxflow workflow create --identity gimp hub://gimp

# Start with defaults, or tune per run with --override
dxflow workflow start gimp
dxflow workflow start gimp \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start gimp --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. GIMP is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach GIMP from anywhere.
