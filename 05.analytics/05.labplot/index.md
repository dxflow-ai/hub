...
```bash
dxflow workflow create --identity labplot hub://labplot

# Start with defaults, or tune per run with --override
dxflow workflow start labplot
dxflow workflow start labplot \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start labplot --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. LabPlot is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach LabPlot from anywhere.
...
steps:
    - name: app
      runtime: docker
      mode: parallel
      image: ghcr.io/dxflow-ai/labplot:latest
      volumes:
          - name: volume
            host: ./volume
            container: /volume
...
- The panel and taskbar are hidden by default so the worksheet fills the screen. Set `PANEL=show` or `TASKBAR=show` to bring back the window decorations and taskbar.
