...
```bash
dxflow workflow create --identity filezilla hub://filezilla

# Start with defaults, or tune per run with --override
dxflow workflow start filezilla
dxflow workflow start filezilla \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start filezilla --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. FileZilla is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach FileZilla from anywhere.
...
steps:
    - name: app
      runtime: docker
      mode: parallel
      image: ghcr.io/dxflow-ai/filezilla:latest
      volumes:
          - name: volume
            host: ./volume
            container: /volume
...
- The panel and taskbar are hidden by default so the transfer window fills the screen. Set `PANEL=show` or `TASKBAR=show` to bring back the window decorations and taskbar.
...
- Transfers land in `/volume`, so pull files down there to keep them across restarts.
