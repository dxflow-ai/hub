...
```bash
dxflow workflow create --identity gabedit hub://gabedit

# Start with defaults, or tune per run with --override
dxflow workflow start gabedit
dxflow workflow start gabedit \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start gabedit --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. Gabedit is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach Gabedit from anywhere.
...
- The panel and taskbar are hidden by default so the workspace fills the screen. Set `PANEL=show` or `TASKBAR=show` to bring back the window decorations and taskbar.
...
- This is the hub's only entry on the Fedora desktop: neither Void nor Ubuntu 26.04 packages Gabedit.
- Gabedit drives other programs rather than computing anything itself — the engines it writes inputs for are not in this image.
