...
```bash
dxflow workflow create --identity lyx hub://lyx

# Start with defaults, or tune per run with --override
dxflow workflow start lyx
dxflow workflow start lyx \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start lyx --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. LyX is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach LyX from anywhere.
...
- Exporting to PDF needs a TeX distribution, which this image does not carry — `xbps-install -Sy texlive` inside the session, or build a variant that bakes it in.
