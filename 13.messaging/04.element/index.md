...
```bash
dxflow workflow create --identity element hub://element

# Start with defaults, or tune per run with --override
dxflow workflow start element
dxflow workflow start element \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start element --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. Element is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach Element from anywhere.
...
- The profile directory lives on the volume, so the session and its encryption keys survive a restart.
- Void packages Element for amd64 only, so this entry is amd64 only.
