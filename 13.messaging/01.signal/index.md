...
```bash
dxflow workflow create --identity signal hub://signal
...
dxflow workflow start signal --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. Signal is already running and maximized — link it to your phone to start. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach Signal from anywhere.
