...
```bash
dxflow workflow create --identity thunderbird hub://thunderbird

# Start with defaults, or tune per run with --override
dxflow workflow start thunderbird
dxflow workflow start thunderbird \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start thunderbird --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. Thunderbird is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach Thunderbird from anywhere.
...
- The panel and taskbar are hidden by default so the client fills the screen. Set `PANEL=show` or `TASKBAR=show` to bring back the window decorations and taskbar.
...
- The profile lives on the volume, so accounts, mail, and settings survive a restart. Void ships the ESR line, which is the conservative one — feature releases land here later than they do upstream.
