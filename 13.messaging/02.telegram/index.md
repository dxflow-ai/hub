---
title: Telegram
description: Telegram messaging client in a remote desktop session
navigation:
    icon: i-diphyx:telegram
---

Telegram is a cloud-based messaging client, streamed here in a remote desktop session and backed by remote compute. It builds on the [Void Desktop](/hub/desktop/void) image — Telegram launches maximized with the window decorations and taskbar hidden, so the app fills the screen.
...
```bash
dxflow workflow create --identity telegram telegram.yml
...
Telegram's data directory is linked into `/volume`, so your session and message history survive restarts — mount a local directory there to keep them.
