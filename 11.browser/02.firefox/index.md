---
title: Firefox
description: Mozilla Firefox web browser in a remote desktop session
navigation:
    icon: i-diphyx:firefox
---

Firefox is Mozilla's open-source web browser, streamed here in a remote desktop session and backed by remote compute. It builds on the [Void Desktop](/hub/desktop/void) image — Firefox launches maximized with the window decorations and taskbar hidden, so the browser fills the screen.
...
```bash
dxflow workflow create --identity firefox hub://firefox

# Start with defaults, or tune per run with --override
dxflow workflow start firefox
dxflow workflow start firefox \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start firefox --link
```

### 2. Open the browser

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. Firefox is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach Firefox from anywhere.
