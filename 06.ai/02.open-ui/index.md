...
- **A reasoning model spends most of its tokens out of sight.** The thinking tags — `qwen3`, `deepseek-r1` and their kin — reason before the first visible word, at the same rate as the answer: one spent 564 tokens on a 60-token reply, 17 seconds where 2 would have done. Turn it off in **Workspace → Models**, the pencil beside the tag → **Advanced Params**, where the `think` field takes `false`. If a chat template override has swallowed it, `/no_think` in the **System Prompt** does it instead.
