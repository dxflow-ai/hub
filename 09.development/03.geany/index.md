...
```bash
dxflow workflow create --identity geany hub://geany

# Start with defaults, or tune per run with --override
dxflow workflow start geany
dxflow workflow start geany \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start geany --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. Geany is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach Geany from anywhere.
...
steps:
    - name: app
      runtime: docker
      mode: parallel
      image: ghcr.io/dxflow-ai/geany:latest
      volumes:
          - name: volume
            host: ./volume
            container: /volume
