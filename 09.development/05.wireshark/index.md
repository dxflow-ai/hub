...
```bash
dxflow workflow create --identity wireshark hub://wireshark

# Start with defaults, or tune per run with --override
dxflow workflow start wireshark
dxflow workflow start wireshark \
    --override env.app.VNC_PASSWORD=my-strong-pass \
    --override env.app.TASKBAR=show

# Publish the web port on an HTTPS link
dxflow workflow start wireshark --link
```

### 2. Open the app

Open your browser at `http://localhost:6082/vnc.html` and enter the password you set in `VNC_PASSWORD`. Wireshark is already running and maximized. Port `5901` is also exposed for connecting a native VNC client. A start given `--link` publishes port `6082` at an HTTPS URL printed on the start line — open it at `/vnc.html` to reach Wireshark from anywhere.
...
- The panel and taskbar are hidden by default so the analyzer fills the screen. Set `PANEL=show` or `TASKBAR=show` to bring back the window decorations and taskbar.
...
- Live capture needs `NET_RAW` and `NET_ADMIN`, which a workflow container is not given — so the interface list comes up empty. Open a `.pcap`/`.pcapng` from `/volume` instead; analysis is what this entry is for.
